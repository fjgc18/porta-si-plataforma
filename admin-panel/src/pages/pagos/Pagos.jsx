...
import { getPagos, createPago, getVentas, getRentas } from '../../services/endpoints'
...
export default function Pagos() {
  ...
  const handleSubmit = async e => {
    e.preventDefault()
    try { await createPago({...form, monto:Number(form.monto), idVenta:form.tipoReferencia==='VENTA'?Number(form.idVenta):null, idRenta:form.tipoReferencia==='RENTA'?Number(form.idRenta):null}); toast.success('Pago registrado'); setOpen(false); load() }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={()=>{setForm({tipoReferencia:'RENTA',idVenta:'',idRenta:'',monto:'',fechaPago:new Date().toISOString().split('T')[0],metodoPago:'EFECTIVO',estado:'PAGADO',concepto:''});setOpen(true)}} className="btn btn-primary"><Plus size={16}/> Registrar Pago</button>
      </div>
      <div className="table-wrap"><table><thead><tr><th>Tipo</th><th>Folio</th><th>Inmueble</th><th>Monto</th><th>Método</th><th>Estado</th><th>Fecha</th></tr></thead>
        <tbody>{data.map(p=><tr key={p.id}><td><span className={`badge ${p.tipoReferencia==='VENTA'?'badge-blue':'badge-gold'}`}>{p.tipoReferencia}</span></td><td className="text-primary-400 font-mono text-xs">{p.folioReferencia}</td><td className="text-white">{p.inmueble}</td><td className="text-emerald-400 font-medium">{formatMoney(p.monto)}</td><td className="text-slate-400 text-xs">{p.metodoPago}</td><td><span className={`badge ${badge[p.estado]||'badge-gray'}`}>{p.estado}</span></td><td className="text-slate-500 text-xs">{formatDate(p.fechaPago)}</td></tr>)}</tbody></table></div>
      {open&&<div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setOpen(false)}><div className="modal"><h3 className="text-lg font-semibold text-white mb-4">Registrar Pago</h3>
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {form.tipoReferencia==='VENTA'?<div><label className="label">Venta</label><select className="input" value={form.idVenta} onChange={e=>setForm({...form,idVenta:e.target.value})} required><option value="">Seleccionar</option>{ventas.map(v=><option key={v.id} value={v.id}>{v.folio} — {v.inmueble}</option>)}</select></div>:<div><label className="label">Renta</label><select className="input" value={form.idRenta} onChange={e=>setForm({...form,idRenta:e.target.value})} required><option value="">Seleccionar</option>{rentas.map(r=><option key={r.id} value={r.id}>{r.folio} — {r.inmueble}</option>)}</select></div>}
